...
from .serializers import (NetworkNodeCreateSerializer, NetworkNodeSerializer, NetworkNodeUpdateSerializer,
                          ProductSerializer)
...
class NetworkNodeViewSet(viewsets.ModelViewSet):
    ...
    def statistics(self, request):
        ...
        total_debt = NetworkNode.objects.aggregate(total_debt=Sum("debt_to_supplier"))["total_debt"] or 0

        cities = NetworkNode.objects.values("city").annotate(count=Count("id")).order_by("-count")[:10]
    ...
    def hierarchy(self, request, pk=None):
        ...
        for client in clients:
            clients_hierarchy.append(
                {"id": client.id, "name": client.name, "type": client.get_node_type_display(), "level": client.level}
            )
